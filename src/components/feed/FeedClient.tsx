        ...
        {stories.map((story) => (
          <button key={story.id} className="flex flex-col items-center gap-1 min-w-fit">
            <Avatar
              size="lg"
              hasStory={true}
              fallback={story.profiles?.username || 'U'}
              src={story.media_url}
            />
        ...
        {initialPosts.map((post) => (
          <Card key={post.id} hover className="p-4">
            ...
            <div className="flex items-center gap-3 mb-3">
              <Avatar size="md" fallback={post.profiles?.username || 'U'} />
              <div className="flex-1">
                <div className="flex items-center gap-1.5">
                  <span className="font-semibold text-sm">{post.profiles?.display_name || post.profiles?.username}</span>
                  <Badge variant="verified">✓</Badge>
                </div>
                <p className="text-xs text-vibe-text-secondary">@{post.profiles?.username} · 2h</p>
              </div>
            </div>
